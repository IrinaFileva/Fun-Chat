...
if(dateUser){
    const greetingUser: HTMLElement = new BaseComponent('p', 'startPage__greeting').addElement(`Hello!<br> ${dateUser.name} ${dateUser.lastName}`);
    setTimeout((): void => {greetingUser.style.opacity = '1'}, 300);
    startPage.append(backgroundImage1, backgroundImage2, titleStartPage, descriptionStartPage, greetingUser);
}
else{
